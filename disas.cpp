...
void disas_code(const object_file &obj, const function &fn, std::vector<uint8_t> &stab) {
    ...
    bool wide{};
    std::size_t labn{};
    auto op = fn.code.begin();
    while (op < fn.code.end()) {
    ...
    while (op < fn.code.end()) {
        ...
        if (lab != labels.end()) {
            ...
            std:: cout << "L" << lab->second << std::endl;
        }
        auto &insn = insns[*op++];
        ...
        case ins_jump: {
            int16_t disp = util::read_either<int16_t>(op, wide);
            wide = false;
            std::cout << " L" << labels[op - fn.code.begin() + disp];
        } break;
        case ins_call: {
            uint16_t disp = util::read_either<uint16_t, uint8_t>(op, wide);
            wide = false;
            std::cout << " " << stab[obj.functions[disp].name];
        } break;
        case ins_local: {
            int16_t disp = util::read_either<int16_t>(op, wide);
            wide = false;
        ...
        case ins_global: {
            uint16_t disp = util::read_either<uint16_t, uint8_t>(op, wide);
            wide = false;
            std::cout << " " << stab[obj.globals[disp].name];
        } break;
        case ins_const:
            switch(op[-1]) {
            case op_ldi_i:
            case op_ldi_l: {
                int16_t cons = util::read_either<int16_t>(op, wide);
                wide = false;
                std::cout << " $" << cons;
            } break;
            case op_ldc_i: {
                auto cons = util::read_next<int32_t>(op);
                std::cout << " $" << cons;
            } break;
            case op_ldc_l: {
...
void disas_object(const object_file &obj, const char *file) {
    ...
    for (const auto &gl : obj.globals) {
        std::cout << ".global " << typid_to_type(gl.type) << " " << &stab[gl.name];
        if (gl.flags & gf_init) {
            const auto ptr = reinterpret_cast<const uint8_t*>(&gl.init_value);
            switch(gl.type) {
            case 'i': std::cout << " " << util::read_at<int32_t>(ptr); break;
            case 'l': std::cout << " " << util::read_at<int64_t>(ptr); break;
            case 'f': std::cout << " " << util::read_at<float>(ptr); break;
            case 'd': std::cout << " " << util::read_at<double>(ptr); break;
            default:
                throw std::logic_error("Oops");
            }
        }
    ...
    for (const auto &fn : obj.functions) {
        /* Function itself */
        std::cout << ".function " << typid_to_type(fn.signature.back()) << stab[fn.name] << std::endl;
