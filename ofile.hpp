...
struct object_file {
    ...
    /* Build raw string table from internal representation
     *
     * @return string table vector
     */
    std::vector<uint8_t> make_strtab() const;
...
static_assert(sizeof(double) == sizeof(int64_t), "double should be same size as int64_t");
static_assert(sizeof(float) == sizeof(int32_t), "float should be same size as int32_t");
